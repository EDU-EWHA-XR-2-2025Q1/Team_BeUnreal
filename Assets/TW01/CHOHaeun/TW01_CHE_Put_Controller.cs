...
public class TW01_CHE_Put_Controller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            int pickCounts = UI.GetComponent<TW01_CHE_UI_Controller>().GetPickCounts();
            if(pickCounts > 0)
            {
                Throw();
                UI.GetComponent<TW01_CHE_UI_Controller>().Decrease_PickCounts();
            }
        }   
    }
